(function() {
  ...
  // last element.
  _.last = function(array, n) { // [1, 2, 3] // expect(_.last([1, 2, 3], 2)).to.eql(3);
    if (n > array.length) {
      return array;
    }
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (isSorted){
      let checked = [];
      _.each(array, function (ele) {
        if (!checked.includes(iterator(ele))) {
          results.push(ele);
          checked.push(iterator(ele))
        }
      })
    } else {
      _.each(array, function (ele) {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(memo, ele) {
      if(!iterator(ele)) {
        return false;
      }
      return memo;
